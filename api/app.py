...
def data():
    ...
    if request.method == 'POST':
        ...
        if (ed[:3] == "MIA"):
            ...
            map = "2"
        try:
            return render_template('results.html', advice=getAdvice(ed, date.day, date.month, date.year, float(threshold), (now.hour+now.minute/60)), location = map, info = [edifici[ed], date.day, date.month, date.year, float(threshold), (now.hour+now.minute/60)])
